...
export default function SignupPage() {
  ...
  useGSAP(
    () => {
      ...
      const split = SplitText.create(".signup-title", { type: "chars" });
      tl.from(split.chars, { opacity: 0, y: 30, stagger: 0.03, duration: 0.5 })
        .fromTo(".signup-card > *", { opacity: 0, y: 16 }, { opacity: 1, y: 0, stagger: 0.07, duration: 0.5 }, "-=0.2");
    },
    { scope: pageRef }
  );
    ...
    <div ref={pageRef} className="min-h-screen flex items-center justify-center bg-zinc-50 px-4 pt-20 pb-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <h1 className="signup-title text-3xl font-light text-zinc-900 mt-6 tracking-tight">
            Create account
          </h1>
          <p className="text-zinc-500 text-sm mt-2">Join Maison for a better experience</p>
        </div>

        <div className="signup-card bg-white p-8 space-y-5 shadow-sm border border-zinc-100">
          {error && (
          ...
          <div className="text-center pt-2">
            <p className="text-sm text-zinc-500">
              ...
              <Link href="/login" className="text-zinc-900 underline underline-offset-4 hover:text-zinc-600">
                Sign in
              </Link>
            </p>
          </div>
